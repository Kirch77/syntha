...
def main():
    ...
    if not database_url:
        ...
        print("   export DATABASE_URL='postgresql://user:pass@localhost:5432/syntha_db'")
    ...
    # Business metrics
    context1.push("business_metrics", {
    ...
    context1.subscribe_to_topics("ProductionAgent", ["production", "monitoring", "alerts"])
    ...
    for i, agent in enumerate(agents):
        context1.push(f"agent_{i}_data", {
            "agent_id": agent,
            "task_count": (i + 1) * 10,
            "status": "active",
    ...
    context1.push("critical_transaction", {
    ...
    print(f"   Transaction status: {transaction['status'] if transaction else 'Not found'}")
    ...
    for user_id in ["enterprise_1", "enterprise_2", "enterprise_3"]:
        ...
        user_contexts[user_id].push("company_data", {
