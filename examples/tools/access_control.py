...
def main():
    ...
    restricted_handler = create_restricted_handler(
        context, 
        "RestrictedAgent", 
        allowed_tools=restricted_tools
    )
    
    restricted_available = [schema['name'] for schema in restricted_handler.get_schemas()]
    ...
    admin_sensitive = admin_handler.handle_tool_call("get_context", keys=["sensitive_data"])
    viewer_sensitive = viewer_handler.handle_tool_call("get_context", keys=["sensitive_data"])
    
    print(f"\n🔒 Admin can see sensitive data: {'sensitive_data' in admin_sensitive['context']}")
    print(f"🔒 Viewer can see sensitive data: {'sensitive_data' in viewer_sensitive['context']}")
