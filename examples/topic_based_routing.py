...
def main():
    ...
    if sales_keys_result['success']:
        ...
        for topic, keys in sales_keys_result['keys_by_topic'].items():
            if keys:  # Only show topics that have keys
                print(f"  📁 {topic}: {keys}")
        print(f"  Total accessible keys: {len(sales_keys_result['all_accessible_keys'])}\n")
    ...
    if marketing_keys_result['success']:
        customer_insights_keys = marketing_keys_result['keys_by_topic'].get('customer_insights', [])
        if customer_insights_keys:
            print(f"MarketingBot found {len(customer_insights_keys)} customer insight keys: {customer_insights_keys}")
            
            # Retrieve all customer insights
            marketing_insights_result = marketing_handler.handle_tool_call(
                "get_context",
                keys=customer_insights_keys
            )
