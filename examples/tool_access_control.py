...
def demonstrate_basic_access_control():
    ...
    result = readonly_handler.handle_tool_call("delete_topic", topic="test", confirm=True)
...
def demonstrate_custom_roles():
    ...
    moderator_handler = create_role_based_handler(mesh, "mod1", "moderator", custom_roles)
...
def demonstrate_multi_agent_setup():
    ...
    agent_configs = {
        ...
        "qa_tester": {"allowed_tools": ["get_context", "push_context", "subscribe_to_topics"]}
    }
    ...
    for agent_name, handler in handlers.items():
        ...
        print(f"    Available tools: {summary['total_available']}/{summary['total_possible']}")
