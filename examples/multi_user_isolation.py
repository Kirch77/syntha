...
def main():
    ...
    alice_sales_handler.handle_tool_call("subscribe_to_topics", topics=["sales", "pricing"])
    alice_support_handler.handle_tool_call("subscribe_to_topics", topics=["support", "tickets"])
    ...
    bob_sales_handler.handle_tool_call("subscribe_to_topics", topics=["sales", "pricing"])
    bob_marketing_handler.handle_tool_call("subscribe_to_topics", topics=["marketing", "campaigns"])
    ...
    bob_marketing_handler.handle_tool_call(
        ...
        value=json.dumps({"email_ctr": 2.5, "social_engagement": 1200, "budget_used": 5000}),
        topics=["marketing"]
    )
    ...
    try:
        ...
        alice_marketing_result = alice_sales_handler.handle_tool_call(
            "get_context",
            keys=["campaign_results"]  # This exists in User 2's context
        )
    ...
    print(f"Alice's available context keys: {list(alice_context.get('context_by_topic', {}).keys())}")
    
    # User 2's available context
    bob_context = bob_sales_handler.handle_tool_call("list_context")
    print(f"Bob's available context keys: {list(bob_context.get('context_by_topic', {}).keys())}")
