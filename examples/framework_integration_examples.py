...
def langchain_integration_example():
    ...
    hybrid_handler = syntha_handler.create_hybrid_handler(
        user_tool_handler=lambda tool_name, **kwargs: getattr(existing_tool_handler, tool_name)(**kwargs)
    )
    
    print("\n🔄 Hybrid Handler Example:")
    print("result = hybrid_handler('get_context', agent_name='DataAnalyst')")
    print("result = hybrid_handler('send_email', to='user@example.com', subject='Test', body='Hello')")
...
def langgraph_integration_example():
    ...
    agent_configs = {
        ...
        "Analyst": {
            "agent_name": "Analyst", 
            "allowed_tools": ["get_context", "push_context", "list_context", "subscribe_to_topics"],
            "role": "contributor"
        },
        "Viewer": {
            "agent_name": "Viewer",
...
def role_based_access_example():
    ...
    roles = {
        "admin": {
            ...
            "tools": ["get_context", "push_context", "list_context", "delete_topic", "send_email", "deploy_app"]
        },
        "contributor": {
            "description": "Can read, write, and manage topic subscriptions",
            "tools": ["get_context", "push_context", "list_context", "subscribe_to_topics", "send_email"]
        },
        "readonly": {
...
def conflict_resolution_example():
    ...
    print("\n🔄 Using renamed tools:")
    print("result = syntha_handler.handle_tool_call('syntha_get_context', agent_name='User')")
