...
def analyze_test_coverage():
    ...
    for test_file in test_files:
        ...
        for module_name in coverage_report:
            if module_name in test_file.name or module_name.replace("_", "") in test_file.name:
                    ...
                    if test_func.startswith("test_"):
                        ...
                        for func in coverage_report[module_name]["functions"]:
                            if func.lower() in tested_name.lower():
                                coverage_report[module_name]["tested_functions"].add(func)
    ...
    for module_name, info in coverage_report.items():
        ...
        func_coverage = (tested_func_count / func_count * 100) if func_count > 0 else 100
        class_coverage = (tested_class_count / class_count * 100) if class_count > 0 else 100
    ...
    # Overall coverage
    overall_func_coverage = (total_tested_functions / total_functions * 100) if total_functions > 0 else 100
    overall_class_coverage = (total_tested_classes / total_classes * 100) if total_classes > 0 else 100
    
    print(f"\n=== Overall Coverage ===")
    print(f"Functions: {total_tested_functions}/{total_functions} ({overall_func_coverage:.1f}%)")
    print(f"Classes: {total_tested_classes}/{total_classes} ({overall_class_coverage:.1f}%)")
...
def analyze_test_quality():
    ...
    for test_file in test_files:
        try:
            ...
            pytest_assertions = [node for node in ast.walk(tree)
                               if isinstance(node, ast.Attribute) and
                               node.attr.startswith('assert')]
    ...
    print(f"Assertions per test: {total_assertions / total_tests:.1f}" if total_tests > 0 else "N/A")
...
def check_test_requirements():
    ...
    total_tested = sum(len(info["tested_functions"]) for info in coverage_report.values())
    ...
    for req_name, result in results.items():
        ...
        if req_name == "coverage":
            print(f"    Required: {result['required']}%, Actual: {result['actual']:.1f}%")
