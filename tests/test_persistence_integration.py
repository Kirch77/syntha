...
class TestPersistenceIntegration:
    ...
    def test_topic_routing_persistence(self):
        ...
        assert self.mesh.get_topics_for_agent("sales_agent") == ["sales", "customer_data"]
        assert self.mesh.get_subscribers_for_topic("sales") == ["sales_agent", "analytics_agent"]
    ...
    def test_agent_permissions_persistence(self):
        ...
        self.mesh.set_agent_post_permissions("senior_agent", ["sales", "analytics", "notifications"])
    ...
    def test_complex_persistence_scenario(self):
        ...
        self.mesh.push("post_restart_data", "System restarted successfully", topics=["sales"])
...
class TestMultiBackendPersistence:
    ...
    def test_sqlite_backend_features(self):
        ...
        try:
            ...
            backend.save_context_item("test_key", "test_value", ["agent1"], 3600, time.time())
...
def run_performance_benchmark():
    ...
    try:
        ...
        start_time = time.time()
        for i in range(1000):
            mesh.push(f"bench_{i}", {"id": i, "data": f"value_{i}", "timestamp": time.time()})
        ...
        print(f"✅ 1000 retrievals: {retrieve_time:.3f}s ({1000/retrieve_time:.0f} ops/sec)")
