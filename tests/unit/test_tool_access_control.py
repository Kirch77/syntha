...
class TestBasicAccessControl:
    ...
    def test_denied_tools_take_precedence(self):
        ...
        handler = ToolHandler(
            self.mesh, "agent1", allowed_tools=allowed_tools, denied_tools=denied_tools
        )
...
class TestToolCallAccessControl:
    ...
    def test_allowed_tool_call_succeeds(self):
        ...
        handler = ToolHandler(
            self.mesh, "agent1", allowed_tools=["get_context", "list_context"]
        )
    ...
    def test_denied_tool_call_fails_with_access_denied(self):
        ...
        assert (
            result["reason"]
            == "Agent 'agent1' does not have permission to use this tool"
        )
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_nonexistent_tool_in_allowed_tools(self):
        ...
        handler = ToolHandler(
            self.mesh, "agent1", allowed_tools=["get_context", "nonexistent_tool"]
        )
    ...
    def test_nonexistent_tool_in_denied_tools(self):
        ...
        handler = ToolHandler(self.mesh, "agent1", denied_tools=["nonexistent_tool"])
    ...
    def test_empty_role_based_access_dict(self):
        ...
        handler = ToolHandler(self.mesh, "agent1", role_based_access={})
    ...
    def test_role_based_access_with_nonexistent_role(self):
        ...
        handler = ToolHandler(
            self.mesh, "agent1", role_based_access={"admin": ["get_context"]}
        )
